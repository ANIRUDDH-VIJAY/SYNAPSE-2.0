...
router.post('/signup', async (req, res, next) => {
  try {
    ...
    // Also return the token and csrf in the JSON response to support
    // clients that prefer Authorization headers or need the CSRF token
...
router.post('/login', async (req, res, next) => {
  try {
    ...
    // Return token and csrf token in JSON as well as setting cookies. This
    // keeps the API backward-compatible while making it easier for the
    // frontend to use Authorization headers in requests if desired.
    return res.json({
      success: true,
      token,
      csrfToken,
